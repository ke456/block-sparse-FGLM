...
\textbf{has rank $\rdim$ if $\seq$ is linearly recurrent} (\todo{I cannot find
this result in the literature..!}). This is showed in \cite[Fact\,1]{KalYuh13}
for sequences having a \emph{scalar} recurrence relation. Let us now link this
with the property of being linearly recurrent as in the above definition.
...
\begin{lemma}
  Consider $\seq = (\seqelt{k})_{k\in\NN} \subset \matSpace[\rdim][\cdim]$ and
  its generating series $\seqpm = \sum_{k>0} \seqelt{k} \var^{-k} \in
  \psMatSpace[\rdim][\cdim]$. Let $\relbas \in \relbasSpace$, and consider
  $\num = \relbas \seqpm$.
  
  is a generating matrix for
  $\seq$ if and only if
