...
floats to do the bulk of the calculation. Explicitly, we rely on
...
though this is by no means a bottleneck here.
...
for matrices with machine float entries. We used OpenMP {\tt parallel
...
less effective. It is an interesting question to clarify this issue
(as the previous paragraph highlights, the key problem is an efficient
parallelization of the loop that computes $(\mU^{\perp} \mM^s)_{0 \le
  s< 2d}$).  Besides, increasing $m$ also increases the time to
...
$\mathsf{BlockParametrizationWithSplitting}$ may take about twice as
...
multiplicities generated randomly had few multiple points, and thus were
