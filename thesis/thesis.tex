...
given element in $\residueI$, say $1,X_1,X_2^2,\dots$ or $1,\lf,\lf^2,\dots$
...
\begin{lemma}\label{lemma:valuelambda}
  Let $\ell$ be in ${\rm hom}_\K(\residueI,\K)$, let $M$ be the minimal
...
\begin{itemize}
	...
	\alpha_{i,1}$;
	\item $\residueI_i$ is a reduced algebra (equivalently, $I_i$ is radical).
\end{itemize}
...
cardinality $1$ and $\residueI_{\sigma_j}$ is reduced.  The algorithm in this
...
\begin{lemma}\label{lemma:acb2}
	...
	linear form over $\prod_{i \in T_j} \residueI_i$ and let $t=t_2 X_2
