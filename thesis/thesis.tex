...
minimal matrix generator  $\mat{P}^{\mU,\mV}$ of it using $O(m^\omega \M(D/m) \log(D/m))
\subset \softO{m^{\omega-1} D}$ operations in $\K$.

Remark as well that for generic $\mU$ and $\mV$, the largest invariant
factor of $\mat{P}^{\mU,\mV}$ is the minimal polynomial $P$ of $\mM$.
...
However, our main algorithm will need to compute numerators of the
form $\Omega((\row{u}_i \mM^s \col{v}_j)_{s \ge 0}, \minpoly)$, where
$\col{v}_j$ is the $j$th column of $\mV$ and $\minpoly$ is the minimal
polynomial of $\mM$. Since $\minpoly$ may have degree $D$, the definition we
gave of $\Omega$ suggests that we may need up to $D$ terms of the
sequence $\row{u}_i \mM^s \col{v}_j$, which we would of course like to
avoid. 

Fix $i$ in $1,\dots,m$ and let $\row{a}_i$ be the row vector defined
by $\row{a}_i = [0~\cdots~0~\minpoly~0~\cdots~0] (\mat{P}^{\mU,\mV})^{-1}$,
...
\begin{proof}
  ...
  invariant factor of $ (\mat{P}^{\mU,\mV})^{-1}$; thus, the product
  $\minpoly (\mat{P}^{\mU,\mV})^{-1}$ has polynomial entries. Since
  $\mat{P}^{\mU,\mV}$ has determinantal degree at most $D$, the bound
