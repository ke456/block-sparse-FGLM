...
same cost as the computation of $Q$. More precisely, the bottleneck
for the algorithm of~\cite{Steel15} is the computation of a
block-Krylov sequence, and we show that once this sequence has been
computed, not only $Q$ but also all other polynomials in the
zero-dimensional parametrization can be computed efficiently, while
...
carry over to matrix sequences. The main results we will need from the
first subsections are \cref{coro:cost_approx} and \cref{randXY}, which
give cost estimates on bla bla
...
$i$.  We also let $\D$ be the dimension of $Q$, so that $\dg \le \D$,
...
For $i$ in $\{1,\dots,\dg\}$, let $Q_i$ be the local algebra at
...
the direct product $Q_1\times \cdots \times Q_\dg$.  We let $N_i$ be the
...
non-singular root of $I$. We also let $\D_i=\dim_\Kbar(Q_i)$, so that
...
$\ell_1,\dots,\ell_\dg$, with $\ell_i$ in ${\rm hom}_\Kbar(Q_i,\Kbar)$
...
\todo{$Q$ and $Q_j$ clash}
All notation, such as $I$, $Q$, \dots, are as in the two previous
...
\todo{find a new letter for the density}
...
$(T-r_j)^{\mu_j}$, and the minimal polynomial of $X_1$ in $Q$ is
$M=\prod_{j \in \{1,\dots,c\}} (T-r_j)^{\mu_j}$.

Recall that a linear form $\ell: Q \to \Kbar$ can be written uniquely
as $\ell=\sum_{i\in \{1,\dots,\dg\}} \ell_i$, with $\ell_i:Q_i \to
...
$X_1: V \to \Kbar$ is one-to-one as in that lemma.

\begin{lemma}\label{lemma:valuelambda}
  Let $\ell$ be in ${\rm hom}_\K(Q,\K)$, let $M$ be the minimal
  polynomial of $X_1$ in $Q$. Then, the polynomial
  $\Omega((\ell(X_1^s))_{s\ge0},M)$ satisfies
...
\begin{proof}
	...
	On the other hand, the minimal polynomial $M$ of $X_1$ can be rewritten as $M=\prod_{j\in
		\mathfrak{e}}(T-r_j) \prod_{j \in \mathfrak{f}}(T-r_j)^{\mu_j}$, so
	that $\delta=\sum_{j \in \mathfrak{e}} 1 + \sum_{j\in \mathfrak{f}}
	\mu_j$. The minimal polynomial
	of the sequence $\ell(X_1^s)$ divides $M$, so that $x_j \le \mu_j$ holds for all $j$ in
	$\mathfrak{f}$.
...
\begin{lemma}\label{lemma:acb2}
	Let $j$ be in $\{1,\dots,c\}$ such that $\mu_j=1$, let $\lambda$ be a
...
factor $P=\prod_{j \in \{1,\dots,c\}, \mu_j=1} (T-r_j)$. We split this
