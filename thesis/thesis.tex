...
\cite[Section~8.4]{GaGe13}.  We assume that the exponent $\omega$ is also valid
for multiplication of matrices over $\polRing$: then, two $m\times m$ matrices
over $\K[T]$ whose degree is less than $d$ can be multiplied using $O(m^\omega
\M(d))$ operations in $\K$.
...
%% FIXME correct cost bound (other locations as well)
...
\begin{proof}
  ...
  \begin{align*}
    ...
    %% TODO parentheses maybe to make it more readable?
...
\begin{algorithm}[H]
  ...
  \item\label{mainstep9}  let $C_1 = \mathsf{ScalarNumerator}(\mat{P}_{\mU,\mV}, \minpoly, \col{\varepsilon}_1, 1, \row{a}_1, 
    (\mat{L}_s)_{0 \le s < d})$
  \item\label{mainstep10} \textbf{for} $i=1,\dots,n$ \textbf{do}
    \begin{enumerate}
     \item let $C_{X_i} = \mathsf{ScalarNumerator}(\mat{P}_{\mU,\mV}, \minpoly, \mM_i\col{\varepsilon}_1, 1, \row{a}_1, (\mat{L}_s)_{0 \le s < d})$
    \end{enumerate}
...
All notation, such as $I$, $\residueI$, it basis
...
\begin{algorithm}[H]
  ...
  \item \textbf{for} $i=0,1,2$ \textbf{do}
    \begin{enumerate}
    \item\label{X1step9}  let $A_i = \mathsf{ScalarNumerator}(\mat{P}_{\mU,\mV}, M, \mN^i \col{\varepsilon}_1, 1, \row{a}_1, (\mat{L}_s)_{0 \le s<d})$
    \end{enumerate}
  \item\label{X1step10} \textbf{for} $i=2,\dots,n$ \textbf{do}
    \begin{enumerate}
     \item let $A_{X_i} = \mathsf{ScalarNumerator}(\mat{P}_{\mU,\mV}, M, \mM_i\col{\varepsilon}_1, 1, \row{a}_1, (\mat{L}_s)_{0 \le s <d})$
    \end{enumerate}
...
\begin{lemma}
  ...
  $((F,G_1,\dots,G_n),X_1)$ of $\mathsf{ParametrizationX}_1$ is a
...
\sum_{i \in \basis} u_i b_i.$$ Then, the above lemma proves that $A_i =
...
Let $\mathfrak{A}$ and $\mathfrak{B}$ be as in the previous section.
The second part of the algorithm computes a zero-dimensional
parametrization of $V_\mathfrak{B}=V-V_\mathfrak{A}$. For this, we are
going to call a modified version Algorithm
$\mathsf{BlockParametrization}$, where we update the values of our
matrix sequences before computing the minimal matrix generator.

We first describe an operation of decomposition for linear forms $\residueI
\to \K$; this is essentially a form of the Chinese Remainder Theorem.
...
\to \Kbar$ and $\ell_\mathfrak{B}: \residueI_\mathfrak{B} \to
\Kbar$ \todo{implicit modulo reduction}. Remark that the support of $\ell_\mathfrak{B}$ is contained in 
$\mathfrak{B}$, and actually equal to $\mathfrak{B}$ for a generic $\ell$.

%% We will let $\ell'=\ell_{\mathfrak{B}}$, so that if $\ell$ is
%% generic, then so is $\ell'$, and calling ${\sf
%%   Parametrization}(\ell,\lf)$, for a random $\lf=t_1 X_1 + \cdots +
%% t_n X_n$, will then return a zero-dimensional parametrization of
%% $V_{\mathfrak{B}}$. In that algorithm, we should of course use the
%% bound $D_\mathfrak{B}$ on the degree of the minimal polynomial.

Suppose that we know $C=\Omega( (\ell(X_1^s))_{s \ge 0}, M)$.

Let $M$ be the minimal polynomial of $X_1$ in $\residueI$ and let 
$F=\prod_{j \in \mathfrak{a}} (T-r_j)$; remark that $F$ divides $M$ 
and is coprime with $M/F$. Since $Q_\mathfrak{A}$ 
is reduced, and since $X_1$ is a separating linear form for 
$V_\mathfrak{A}$, we have
$Q_\mathfrak{A} \simeq \K[T]/F.$ The generating series
\begin{align*}
...
In the following table, we give a ratio of the timing for $\mathsf{ParametrizationX}_1$
...
For all inputs, with the exception of sot1, $X_1$ separates most of the
