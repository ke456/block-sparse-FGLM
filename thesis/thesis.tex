...
as before, the cost is
$ O(m^\omega \M(D/m) \log(m) + m \M(D)\log(D)\log(m)) \subset \softO{m^{\omega-1}D}$ operations in $\K$.
...
\begin{algorithm}[H]
  \caption{$\mathsf{ParametrizationWithSplitting}(\mM_1,\dots,\mM_n,\mU,\mV,\lf$)}
	...
	\begin{enumerate}
		...
		\item let $(R,W_1,\dots,W_n,\lf)=\mainalgoname{\sf Residual}(\mM_1,\dots,\mM_n,\mU,\mV,(\mat{\Delta}_s),(\mat{\Delta}'_s),(\mat{\Delta}''_s),\lf)$
...
We then apply a modified version of Algorithm $\mainalgoname$, called
$\mainalgoname{\sf Residual}$, which computes a zero-dimensional
...
$i=1,\dots,m$; this takes $ O(m^{\omega+1} \M(D/m) \log(m) + m^2
\M(D)\log(D)\log(m))$ operations in $\K$, using the analysis of
\cref{ssec:scalar_numer}. Then, the total time spent in {\sf
...
\begin{algorithm}[H]
  ...
  \item\label{residualstep10} \textbf{for} $i=1,\dots,n$ \textbf{do}
    \begin{enumerate}
     \item let $C_{X_i} = \mathsf{{\sf ScalarNumeratorCorrected}}(\mat{S}_{\mU,\mV},S, \mM_i\col{\varepsilon}_1, 1, \row{a}_1, (\mat{D}_s)_{0 \le s < d_\mathfrak{B}}, (\mat{\Delta}'_s)_{0 \le s < d_\mathfrak{B}})$
    \end{enumerate}
...
the machine used for timings are the same as section \ref{section:ex}.
