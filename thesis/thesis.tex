...
Let $c$ be the sparsity of $M$, then generating $(u_k^{tr} \mM^i)_{0 \le i < 2d}$ costs 
$O(d cD^2) = O(\frac{cD^3}{m} )$ field operations. If we are able to compute $m$ 
vector-matrix products in parallel at once, the total cost of generating $(\mU^{tr}\mM^i)_{0 \le i < 2d}$
is $O(\frac{cD^3}{m})$. We can then compute $(\mU^{tr}\mM^i\mV)_{0 \le i < 2d}$ by a product
$$
\begin{bmatrix}
\mU^{tr}\\
\mU^{tr} \mM\\
...
can compute $N,N_1,\dots,N_j$ in cost $O(nmD)$. Since $P,R,N,N_j$, $1\le j \le n$
...
\begin{tabular}{c|c|c|c|c|c|c}
	\textbf{name}& $\mathbf{D}$ & \textbf{Sparsity} & $\mathbf{m = 1}$ & $\mathbf{m = 4}$ & $\mathbf{m = 8}$ & $\mathbf{deg(P) = D}$\\
	\cline{1-7}
	...
	randsq3-2 & 4096& 0.04     & 699.944 & 187.988 & 128.88  & no  \\
...
By corollary 16 of \cite{Stor03}, the cost of this is $O(D^\omega \log(D))$.
