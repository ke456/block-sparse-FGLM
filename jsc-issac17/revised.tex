...
Gilles Villard for useful discussions. This research was partially supported by
NSERC (Schost's NSERC Discovery Grant) and by the CNRS-INS2I Institute through
its program for young researchers (Neiger's project ARCADIE).
...
$\mathcal{L}$ is known to have order at most $d$ and that we are given
its first $2d$ terms, we can recover its minimal polynomial $P$ by
means of the Berlekamp-Massey algorithm, or of Euclid's algorithm
applied to rational function reconstruction; this can be done in
$O(\M(d)\log(d))$ operations in $\K$ \citep{BrGuYu80}. Given $P$ and
$\ell_0,\dots,\ell_{\deg(P)-1}$, we can deduce $\Omega(\mathcal{L},P)$
...
an improved complexity analysis). Proofs of the main statements are in
appendix.
...
Given a linearly recurrent sequence, our goal is to find a minimal
left matrix generator of it. In order to do this, we will assume that
we know bounds on the degree of this left generator, but also on that
of a {\em right} generator. Indeed, in all the previous discussion, we
may also consider vector relations operating on the right. In
particular, \cref{lem:module_rank} shows that if a sequence is
linearly recurrent, these right relations form a submodule of
$\polMatSpace[\rdim][1]$ of rank $\rdim$, so that a linearly recurrent
sequence also admits right generators.
...
minimal right generators. Knowing bounds $(\degBdl,\degBdr)$ on these
...
Wiedemann algorithm~\citep{Coppersmith94}. 
...
\textcolor{red}{All our inputs as well as our source code are available at
...
(such as $1,X_1,X_2^2,\dots$ or $1,\lf,\lf^2,\dots$) and
linear algebra over the univariate polynomials, as we did before, as well as some operations on
...
In this appendix, we prove results claimed in
Sections~\ref{section:matrix_seq} and~\ref{ssec:appliW}, starting with
Theorem~\ref{coro:cost_approx}. The first lemma we need is similar to
\citep[Theorem~4.5]{Turner02}.
\begin{lemma}
...
\begin{proof}
  ...
  $\seq$. The two items in the theorem follow.
\end{proof}
...
Using the fast approximant basis algorithm of~\citep{GiJeVi03},
Theorem~\ref{coro:cost_approx} follows. Next, we discuss
