...
favorable inputs, these algorithms fall back on the
...
lines of Wiedemann's algorithm to solve sparse linear systems
\cite{Wiedemann86}. These techniques have also been widely used in integer
factorization or discrete logarithm calculations, going back to~\cite{LaOd90}.
It has become customary to design block versions of such algorithms to
parallelize their bottleneck, as pioneered by Coppersmith~\cite{Coppersmith94}
in the context of integer factorization: it is then natural to adapt this
strategy to our situation. This was already discussed by Steel
...
precisely, the bottleneck of the algorithm of~\cite{Steel15} is the
...
the libraries LinBox~\cite{LinBox}, Eigen~\cite{Eigen} and NTL~\cite{NTL}.
...
refined versions of results from~\cite{BoSaSc03} that will be 
...
(see~\cite{Villard97,Villard97a,KalVil01,Turner02,KaVi04}), but the cost
...
\begin{definition}
  ...
  \begin{itemize}
  \item \emph{minimal} if $\mat{P}$ is in reduced form \cite{Wolovich74,Kailath80};
...
respectively~\cite{GiJeVi03},~\cite{ZhoLab12}, and~\cite{JeaNeiVil18}.
...
found in~\cite{Villard97a} and~\cite{KaVi04} (see also~\cite{Kaltofen95}
...
\begin{proof}
 ...
 has full dimension $D_\mU$ for a
  ...
  polynomial of the ``generic'' sequence $(\trsp{\scrU} \mM^s)_{s \ge
...
$\mat{P}_{\mU,\mV}$ using an algorithm such as
Storjohann's~\cite[Section~17]{Stor03}, yet the cost would be slightly higher on the
...
nonzero; by~\cite[Lemma~3.3]{Mourrain97} we have $w \le
N_i-1$. Then, we let
...
\in [0,1]$, that is, at most $\density D^2$ nonzero entries.  As a
...
Next, we compute the row vector $\row{a}_1$ and the numerator $\mat{\Omega}( (\mUt \mM^s\col{\varepsilon}_1)_{s \ge 0}, \mat{P}_{\mU,\mV})$:
\begin{align*}
\row{a}_1 &= [T+16,13]\\
...
The latter is made from the entries of nonnegative degree in the product
$$\mat{P}_{\mU,\mV} 
\left (
\begin{bmatrix}
84\\  
...
and we obtain, after dot product with $\row{a}_1$,
\begin{align*}
...
implementation is based on Shoup's NTL \cite{NTL} polynomials, LinBox
...
as is done in~\cite{fflas-ffpack} for dense matrices, we use machine
...
to a lex ordering) in that paper's algorithm.  Rather than optimizing our
...
\newblock {\em J. Symbolic Comput.}, 70:49--70, 2015.
...
\newblock Radical computations of zero-dimensional ideals and real root
  counting.
\newblock {\em Mathematics and Computers in Simulation}, 42(4-6):561--569,
  1996.
...
\newblock {Polynomial systems solving by fast linear algebra}.
...
\newblock Efficient computation of zero-dimensional {G}r\"obner bases by change
  of ordering.
\newblock {\em J. Symbolic Comput.}, 16(4):329--344, 1993.

\bibitem{FaMo17}
J.-C. Faug{\`e}re and C.~Mou.
\newblock Sparse {FGLM} algorithms.
\newblock {\em J. Symbolic Comput.}, 80(8):538--569, 2017.

\bibitem{FaSaSp13}
J.-C. Faug\`ere, M.~{Safey El Din}, and P.-J. Spaenlehauer.
\newblock On the complexity of the generalized {M}in{R}ank problem.
\newblock {\em J. Symbolic Comput.}, pages 30--58, 2013.

\bibitem{fflas-ffpack}
FFLAS-FFPACK-Team.
\newblock {\em {FFLAS-FFPACK}: {F}inite {F}ield {L}inear {A}lgebra
  {S}ubroutines / {P}ackage}, v2.2.2 edition, 2016.
...
\bibitem{JeaNeiVil18}
C.-P. Jeannerod, V.~Neiger, and G.~Villard.
\newblock Fast computation of approximant bases in canonical form.
\newblock Accepted for publication in {\em J. Symbolic Comput.}, 2018.
...
\newblock {\em Mathematics of Computation}, 64(210):777--806, 1995.
...
E.~Kaltofen and G.~Villard.
\newblock On the complexity of computing determinants.
\newblock {\em Comput. Complexity}, 13(3-4):91--130, 2004.

\bibitem{Keller85}
W.~Keller-Gehrig.
\newblock Fast algorithms for the characteristic polynomial.
\newblock {\em Theoret. Comput. Sci.}, 36(2-3):309--317, 1985.
...
\newblock {\em Trans. Amer. Math. Soc.}, 348(8):3283--3321, 1996.

\bibitem{MorenoSocias91}
G.~Moreno-Soc{\'\i}as.
\newblock {\em Autour de la fonction de {H}ilbert-{S}amuel (escaliers
  d'id{\'e}aux polynomiaux)}.
\newblock PhD thesis, \'Ecole polytechnique, 1991.

\bibitem{Morgan88}
A.~Morgan.
\newblock {\em Solving Polynominal Systems Using Continuation for Engineering
...
\bibitem{Neiger16}
V.~Neiger.
\newblock {\em {Bases of relations in one or several variables: fast algorithms
  and applications}}.
...
\newblock {\em J. Symbolic Comput.}, 50:110--138, 2013.

\bibitem{Rouillier99}
F.~Rouillier.
\newblock Solving zero-dimensional systems through the {R}ational {U}nivariate
  {R}epresentation.
\newblock {\em Appl. Algebra Engrg. Comm. Comput.}, 9(5):433--461, 1999.

\bibitem{Sakata90}
S.~Sakata.
\newblock Extension of the {B}erlekamp-{M}assey algorithm to {$N$} dimensions.
\newblock {\em Information and Computation}, 84(2):207--239, 1990.
...
\newblock {\em J. Symbolic Comput.}, 17(5):371--391, 1994.
...
\newblock {\em J. Symbolic Comput.}, 36:613--648, 2003.

\bibitem{Turner02}
W.~J. Turner.
\newblock {\em {Black box linear algebra with the LINBOX library}}.
...
\bibitem{Wiedemann86}
D. Wiedemann.
\newblock {\em Solving sparse linear equations over finite fields}.
\newblock IEEE Trans. Inf. Theory IT-32, pp. 54--62, 1986.

\bibitem{Wolovich74}
W.~A. Wolovich.
\newblock {\em Linear Multivariable Systems}, volume~11 of {\em Applied
